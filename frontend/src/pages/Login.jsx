...
function Login() {
  ...
  return <div className="login-container"><form className="login-card" onSubmit={submit}><h1>TrafficVision AI</h1><p className="subtitle">Smart Traffic Prediction & Congestion Management</p><h2>{signup ? "Create an account" : "Welcome back"}</h2><input type="email" placeholder="Email address" value={email} onChange={(e) => setEmail(e.target.value)} required /><input type="password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} required />{signup && <select value={role} onChange={(e) => setRole(e.target.value)}><option>Public User</option><option>Traffic Operator</option><option>Admin</option></select>}{error && <p className="error">{error}</p>}<button disabled={loading}>{loading ? "Please wait…" : signup ? "Sign up" : "Login"}</button><button type="button" className="login-switch" onClick={() => { setSignup(!signup); setError(""); }}>{signup ? "Already have an account? Login" : "New here? Create an account"}</button></form></div>;
}
