...
def _nearest_road_congestion(db: Session, lat: float, lon: float) -> str | None:
    """Find whichever monitored road is closest to a point (within threshold)
    and return its current congestion level, so route time estimates reflect
    OUR live data, not just OSRM's static routing graph."""
    roads = (
    ...
    return latest.congestion_level if latest else None
...
def plan_route(
    ...
    origin_level = _nearest_road_congestion(db, origin_lat, origin_lon)
    dest_level = _nearest_road_congestion(db, dest_lat, dest_lon)
    levels = [l for l in (origin_level, dest_level) if l]
    worst_level = "low"
    for level in levels:
        if level == "high":
            worst_level = "high"
            break
        if level == "medium" and worst_level != "high":
            worst_level = "medium"
