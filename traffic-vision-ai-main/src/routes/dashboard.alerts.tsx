...
import { emailAlerts, updateAlertStatus } from "@/lib/traffic.functions";
...
function Alerts() {
  ...
  const sendDigest = useServerFn(emailAlerts);
  const mailAlerts = useMutation({
    mutationFn: () => sendDigest(),
    onSuccess: (res) => toast.success("Alert digest emailed", { description: `${res.count} alert(s) sent to ${res.recipient}` }),
    onError: (e: Error) => toast.error("Could not email alerts", { description: e.message }),
  });
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            <Button variant="outline" disabled={mailAlerts.isPending} onClick={() => mailAlerts.mutate()}>
              <Mail className="mr-1.5 h-4 w-4" />Email alerts to me
            </Button>
            <Button className="bg-brand text-primary-foreground">Create alert rule</Button>
          </>
        }
      />
