# TrafficVision AI — Milestones 1, 2 & 3 Complete
...
- **Milestone 3** (Week 5–6): Alerts, Analytics & AI Insights — **done**
...
| `GET /prediction/report/{road_id}/download?hours=24` | any authenticated user | Same report as a downloadable PDF file (generated with ReportLab — title, summary table, peak/quietest hour, and full hour-by-hour forecast table) |
...
**Also note:** both the Navigation page's route map and the standalone Live
Map page load the Leaflet library itself from `unpkg.com` at runtime. In a
network-restricted environment (like the sandbox this project was built and
tested in) that CDN request is blocked, so both map pages show a clear
"couldn't load the map library" message instead of a blank/broken screen —
this was confirmed directly. On a machine with normal internet access this
resolves itself with no code changes needed.
...
## Alert & Notification Module (Milestone 3 — complete)

All four alert types the spec calls for (congestion, accident, route delay, emergency) are implemented and reachable — not just defined as enum values.

| Alert type | How it's raised |
|---|---|
| **Congestion alerts** | Fully automatic. A background thread (`_monitor_congestion_alerts` in `main.py`) checks every monitored road's latest reading every 10s and opens an alert the moment it hits `high`, auto-resolving it once congestion eases. No duplicate alerts while one's already open for that road. |
| **Accident notifications** | Manual, by design — real accidents need a human to report them. `POST /alerts/accidents` (admin/operator). |
| **Route delay warnings** | Automatic, triggered from `GET /routes/plan`. If the recommended route passes near one of our own monitored roads currently at medium/high congestion, a `route_delay` alert is raised for that road, with the estimated extra minutes. Deduped with a 15-minute cooldown per road so repeatedly planning routes through the same jam doesn't spam alerts. See `_raise_route_delay_alert_if_needed` in `routing.py`. |
| **Emergency traffic alerts** | Two paths: (1) manual broadcast via `POST /alerts/emergency` (admin/operator) — road closures, natural disasters, VIP movement, evacuations, etc., optionally tied to a specific road or left citywide (`road_id` omitted); (2) automatic citywide gridlock detection — if ≥40% of monitored roads (minimum 3) are simultaneously at `high` congestion, the same background thread raises a single citywide `emergency` alert instead of spamming N separate congestion alerts, and auto-resolves it once the ratio drops back down. |

| Endpoint | Access | Purpose |
|---|---|---|
| `GET /alerts?status=&alert_type=&limit=` | any authenticated user | List alerts, filterable by status (`active`/`acknowledged`/`resolved`) and type (`congestion`/`accident`/`route_delay`/`emergency`) |
| `GET /alerts/active-count` | any authenticated user | Count of unresolved alerts (dashboard badge) |
| `POST /alerts/accidents` | admin, operator | Report an accident on a road |
| `POST /alerts/emergency` | admin, operator | Broadcast an emergency alert (road-specific or citywide) |
| `PUT /alerts/{id}/acknowledge` | admin, operator | Mark an alert as seen/being handled |
| `PUT /alerts/{id}/resolve` | admin, operator | Close out an alert |

## Analytics Dashboard Module (Milestone 3 — complete)

| Endpoint | Purpose |
|---|---|
| `GET /analytics/summary` | Traffic analytics report: platform-wide snapshot — congestion breakdown, avg speed, active alerts/accidents |
| `GET /analytics/road-performance` | Road performance tracking: per-road averages + a 0–100 performance score |
| `GET /analytics/heatmap` | Congestion heatmaps: per-road congestion intensity across sequential time windows |
| `GET /analytics/trends` | Traffic trend analysis: hour-of-day / day-of-week congestion patterns, plus which roads are improving vs. worsening |

## AI Prediction / Insights Module (Milestone 3 — complete)

| Endpoint | Purpose |
|---|---|
| `GET /insights/recommendations` | Smart traffic recommendations per road — current congestion + (where a trained model exists) a forward-looking warning from the forecasting model |
| `GET /insights/patterns/{road_id}` | Pattern analysis for a single road |

**Milestone 3 is now complete**: alert/notification workflows (all four types), analytics/heatmap dashboards, and AI-based recommendations are all implemented, wired into the frontend, and tested against a running server.

### Known limitation: OSRM/Nominatim still untested against live internet

This carries over from Milestone 2 and directly affects `route_delay` alerts, since they're triggered from `/routes/plan`. Confirmed directly in this environment: both `nominatim.openstreetmap.org` and `router.project-osrm.org` are blocked by network egress rules (`403 host_not_allowed`) — not a bug in the code, a sandbox restriction. What **was** verified end-to-end:
- The blocked-network case is handled cleanly: both endpoints return a proper `400` with a clear message, no crash.
- The `route_delay` alert-creation logic itself was tested directly (mocking the OSRM response) — it correctly raises an alert on medium/high congestion near a route endpoint, attributes it to the right road, and respects the 15-minute dedupe cooldown.

What still needs a real smoke test once you have unrestricted internet access:
```
curl "http://localhost:8000/routes/geocode?query=Silk+Board+Junction" -H "Authorization: Bearer <token>"
curl "http://localhost:8000/routes/plan?origin_road_id=1&destination_road_id=2" -H "Authorization: Bearer <token>"
```
...
├── backend/
│   ├── app/
│   │   ├── main.py        # FastAPI app, CORS, startup seed, sensor-feed simulator, alert monitor thread
│   │   ├── database.py    # SQLAlchemy engine (SQLite by default, Postgres-ready)
│   │   ├── models.py      # User, Road, TrafficReading, Alert
│   │   ├── schemas.py     # Pydantic request/response models
│   │   ├── security.py    # Password hashing, JWT, RBAC dependency
│   │   ├── seed.py        # Bootstraps demo admin/operator + 4 sample roads
│   │   ├── routing.py     # Route Analysis Module: OSRM/Nominatim + route_delay alerts
│   │   ├── prediction.py  # Traffic Prediction Module: RandomForest train/forecast/report
│   │   └── routers/
│   │       ├── auth.py       # /auth/register, /auth/login
│   │       ├── users.py      # /users/me (profile), /users (admin list — RBAC demo)
│   │       ├── traffic.py    # /traffic/roads, /traffic/readings, /traffic/live
│   │       ├── prediction.py # /prediction/train, /forecast, /report
│   │       ├── routes.py     # /routes/geocode, /routes/plan, /routes/road-condition
│   │       ├── alerts.py     # /alerts, /alerts/accidents, /alerts/emergency
│   │       ├── analytics.py  # /analytics/summary, /road-performance, /heatmap, /trends
│   │       └── insights.py   # /insights/recommendations, /insights/patterns
│   └── requirements.txt
├── frontend/
│   ├── index.html         # Login screen
│   ├── dashboard.html      # Live monitoring, prediction, routes, alerts, analytics, AI insights pages
│   ├── app.js              # Polling, RBAC-aware rendering, all module pages
...
| POST | `/prediction/train/{road_id}` | admin, operator | Train/retrain the forecasting model for a road |
...
| GET | `/routes/plan` | any authenticated user | Alternate routes + congestion-aware ETA (also triggers route_delay alerts) |
| GET | `/routes/road-condition/{road_id}` | any authenticated user | Current condition for a road |
| GET | `/alerts` | any authenticated user | List alerts (filterable) |
| POST | `/alerts/accidents` | admin, operator | Report an accident |
| POST | `/alerts/emergency` | admin, operator | Broadcast an emergency alert |
| GET | `/analytics/summary` | any authenticated user | Platform-wide analytics snapshot |
| GET | `/analytics/heatmap` | any authenticated user | Congestion heatmap |
| GET | `/insights/recommendations` | any authenticated user | AI-based smart traffic recommendations |

## Next milestone (not built yet, by design)

- **Milestone 4** (Week 7–8): full automated test suite, Docker/cloud deployment, final documentation & demo.
